...
class SubprocessNetWorker(QThread):
    ...
    def __init__(self, network: Network, exe_path: str):
        """
        Initialize the SubprocessNetWorker with a network and executable path.
...
class SubprocessNetWorkerFactory(Protocol):
    """
    Protocol for a callable that creates instances of SubprocessNetWorker.
    """

    def __call__(self, network: Network) -> SubprocessNetWorker: ...
